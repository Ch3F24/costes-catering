...
<nav @class([
        ...
        'top-0 px-4 z-50 w-full grid grid-cols-5 md:block items-center transition-all duration-[175ms] md:duration-300 ease-in-out pb-4 bg-white'
    ])
    @if($transparent)data-nav="transparent" @endif
     x-data="{ show: false}"
     x-bind:class="{ 'nav-open': show  }"
>

    <div class="mobil_menu-triggers md:hidden">
        <svg x-show="!show" id="menu-show" class="text-black"  data-name="Outlined/UI/menu" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" @click="show = true">
